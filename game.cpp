...
void mouseActions(int button, int state, int x, int y)
{
	if(button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) { 
		if((x>420 && x<490) && (y>459 && y<491)){   // for exiting the application
/*      		system("g++ bfs2.cpp");
      		system("./a.out");
...
void display() {
	...
	float points5[4][2] = {{420,20},{490,20},{490,50},{420,50}}; 
	...
	if(nextlevel==1){
		...
		glEnd();*/
		/*char m4[100]="Next";*/
/*		char m4[100]="Going to next level in 3 second ";

		int l4 = (int)strlen(m4);
		glRasterPos2f(150,40);
		for(int i = 0 ; i < l4; i++)
